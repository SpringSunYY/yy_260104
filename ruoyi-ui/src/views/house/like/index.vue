<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="名称" prop="houseTitle">
        <el-input
...
<!--      </el-col>-->
    ...
    <el-table :loading="loading" :data="likeList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="房源标签" align="center" :show-overflow-tooltip="true" v-if="columns[9].visible"
                       prop="tags"/>
      <el-table-column label="分数" align="center" :show-overflow-tooltip="true" width="200" v-if="columns[10].visible"
                       prop="score">
        <template slot-scope="scope">
          <span v-if="scope.row.tags && scope.row.tags.trim() !== ''" >
            <el-tag
      ...
      <el-table-column label="创建时间" align="center" :show-overflow-tooltip="true" v-if="columns[11].visible"
                       prop="createTime"/>
...
import {listLike, getLike, delLike, addLike, updateLike} from "@/api/house/like";
...
export default {
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/house/like/importData"
      },
      // 表单校验
      rules: {
        id: [
  ...
  methods: {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('house/like/export', {
        ...this.queryParams
    ...
    buildSubmitData() {
      ...
      if (data.houseId !== null && data.houseId !== undefined && data.houseId !== "") {
        data.houseId = parseInt(data.houseId, 10);
      } else {
        data.houseId = null;
      }
